...
local UpItemEntry=70000
local CzClItemCountBl=0.3      --重置需要原升级材料的倍率
local chaijieItemCountBl=0.5   --拆解获取原升级材料的倍率
local LuckyPointPCH=0.004  --表示杀一只提供经验或者荣誉值的怪物提供多少幸运点
...
function Item_upHello(event, player, item, target)
		...
		--幸运点函数
		LuckyPointTemp=0
		UseLuckyPoint=CharDBQuery("SELECT UseLuckyPoint FROM characters_jf WHERE guid="..pGuid)
		...
		targetCX[pGuid]=WorldDBQuery("SELECT * FROM item_up WHERE ENTRY ="..targetID[pGuid])
		targetCZ[pGuid]=WorldDBQuery("SELECT * FROM item_up WHERE chengpin ="..targetID[pGuid])--搜寻重置装备上一级装备id
...
function re_add_item(event, player, item, target)  --装备重置
	...
	if (player:GetItemCount(targetID[pGuid]) >1) then
		...
		player:SendAreaTriggerMessage("装备重置失败，请检查背包，当前"..GetItemLink(targetID[pGuid]).."有"..player:GetItemCount(targetID[pGuid]).."件放置在您的背包中。")
			...
			local cz1_TorF,cz2_TorF,cz3_TorF,cz4_TorF,cz5_TorF,czSelf_TorF
...
function item_up_en(event, player, item, sender, intid, code)
	pGuid=player:GetGUIDLow()
		...
		if(H1_TOrF and H2_TOrF and H3_TOrF and H4_TOrF and H5_TOrF) then
			...
			CharDBExecute("update characters_jf set UseLuckyPoint=UseLuckyPoint+"..LuckyPointTemp.." where guid="..pGuid..";")
		else
