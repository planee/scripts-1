...
local itemEntry=70008 --物品id,需要那些可以点击使用的物品，比如炉石之类的
local qtos=0.00001   --每完成多少个任务获得一点分配点数
local cz_item=70001   --重置加点所需要的物品材料
local cz_item_count=5  --重置所需要材料数量

local tj=3631     --表明该所要统计的参数，下面列出参数，当然条件参数远不止这么点，不过我就随便列出两个算了，当然如果改动为杀敌的话，下面的部分说明文字请自行改正。
--[[
3631		---任务数量
5529		---杀敌数量
]]--
...
local function stat_AddGoss(event, player, item, target,intid)	
	...
	for k,v in pairs(goss) do 	
		...
		player:GossipMenuAddItem(icon,text,1,intid)			
			...
			player:GossipMenuAddItem(0,"\n>>  力量 + |cFF006699"..points:GetUInt32(2).."|r",1,1)		
			player:GossipMenuAddItem(0,"\n>>  敏捷 + |cFF006699"..points:GetUInt32(3).."|r",1,2)	
			player:GossipMenuAddItem(0,"\n>>  耐力 + |cFF006699"..points:GetUInt32(4).."|r",1,3)	
			player:GossipMenuAddItem(0,"\n>>  智力 + |cFF006699"..points:GetUInt32(5).."|r",1,4)	
			player:GossipMenuAddItem(0,"\n>>  精神 + |cFF006699"..points:GetUInt32(6).."|r",1,5)	
		...
		questcount={0,0,0,0}
		player:SendBroadcastMessage("当前完成任务数量为0，请至少完成一个任务开启功能。")			
	end			
end
